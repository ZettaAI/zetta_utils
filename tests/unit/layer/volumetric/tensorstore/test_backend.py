...
write_info_notmock = cloudvol.backend.PrecomputedInfoSpec.write_info
...
def test_ts_backend_info_overwrite(clear_caches, path, reference, mode, mocker):
    ...
    info_spec.write_info.assert_called_once()
...
def test_ts_backend_write_exc(data_in, expected_exc, clear_caches, mocker):
    ts_m = mocker.MagicMock()
    ts_m.__setitem__ = mocker.MagicMock()
    mocker.patch("cloudvolume.CloudVolume.__new__", return_value=ts_m)
    tsb = TSBackend(path=LAYER_X0_PATH)
    index = VolumetricIndex(
...
def test_ts_get_chunk_size(clear_caches):
    cloudvol.backend.PrecomputedInfoSpec.write_info = write_info_notmock
    info_spec = cloudvol.backend.PrecomputedInfoSpec(
        reference_path=LAYER_X0_PATH,
