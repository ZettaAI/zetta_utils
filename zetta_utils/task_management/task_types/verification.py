...
VerifierFunc = Callable[[str, Task, str], "VerificationResult"]
...
def verify_task(project_name: str, task: Task, completion_status: str) -> VerificationResult:
    ...
    if not verifier:
        return VerificationResult(
            passed=True, message=f"No verifier registered for task type: {task['task_type']}"
        )

    return verifier(project_name, task, completion_status)
