        ...
        try:
            ...
            for sema_type in semaphores_spec_:
                try:
                    ...
                    raise RuntimeError(
                        f"Failed to pre-create shared memory for tracking `{sema_type}`"
                    ) from e
...
def reset_all_timing_data() -> None:
    ...
    for sema_type in get_args(SemaphoreType):
        ...
        except (FileNotFoundError, struct.error, AttributeError) as e:
            raise RuntimeError(f"Failed to reset timing data for {sema_type}") from e
...
def get_all_timing_data(
    ...
    for sema_type, sema_count in semaphores_spec.items():
        try:
            ...
            # Calculate runtime from the earliest start time
    ...
    for sema_type in semaphores_spec.keys():
        if sema_type in result:
            ...
            # New runtime utilization percentage
            # Total potential lease time = num_semaphores * runtime
            if runtime > 0 and sema_count > 0:
                total_potential_lease_time = sema_count * runtime
                runtime_utilization = lease_time / total_potential_lease_time * 100
                result[sema_type]["runtime_utilization_percentage"] = runtime_utilization
            else:
                result[sema_type]["runtime_utilization_percentage"] = 0.0
...
def _log_timing_summary(  # pylint:disable=too-many-locals, too-many-statements
    ...
    for sema_type, stats in timing_data.items():
        if isinstance(stats, dict) and "lease_count" in stats:
            ...
            if lease_count > 0:
                ...
                sema_count = stats.get("semaphore_count", 0)
                total_wait = stats.get("total_wait_time", 0.0)
                total_lease = stats.get("total_lease_time", 0.0)
                avg_wait = stats.get("average_wait_time", 0.0)
                avg_lease = stats.get("average_lease_time", 0.0)
    ...
    if summary_stats:
        summary += lrpad("", filler="-", bounds="|", length=80) + "\n"
        total_acquisitions = summary_stats.get("total_acquisitions", 0)
        total_wait_all = summary_stats.get("total_wait_time_all_semaphores", 0.0)
        total_lease_all = summary_stats.get("total_lease_time_all_semaphores", 0.0)
        overall_avg_wait = summary_stats.get("overall_average_wait", 0.0)
        overall_avg_lease = summary_stats.get("overall_average_lease", 0.0)
    ...
    runtime = summary_stats.get("runtime", 0.0) if summary_stats else 0.0
    if runtime > 0:
        summary += lrpad("", bounds="|", length=80) + "\n"
        ...
        for sema_type, stats in timing_data.items():
            if isinstance(stats, dict) and "lease_count" in stats:
                if stats.get("lease_count", 0) > 0:
                    sema_count = stats.get("semaphore_count", 0)
                    total_capacity = sema_count * runtime
                    utilization = stats.get("runtime_utilization_percentage", 0.0)
