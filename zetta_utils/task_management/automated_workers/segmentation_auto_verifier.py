...
def log_retry_attempt(retry_state):
    ...
    if retry_state.outcome is not None and retry_state.outcome.failed:
        ...
        logger.warning(f"Skeleton fetch attempt {attempt_number} failed with: {exception}. Retrying...")
        console.print(f"[yellow]⚠️  Skeleton unavailable (attempt {attempt_number}), retrying...[/yellow]")
...
def get_skeleton(ng_dict: dict, graphene_path: str, ng_resolution: list, sv_resolution: list) -> SegmentSkeleton | None:
...
def process_task(task_id: str, project_name: str, user_id: str, 
                min_skeleton_length_mm: int, slack_channel: str, task_count: int,
                graphene_path: str, ng_resolution: list, sv_resolution: list, slack_users: tuple) -> None:
    ...
    trace_task_id = task_details['extra_data']['trace_task_id'] 
    ...
    timesheet_summary = get_task_timesheet_summary(project_name=project_name, task_id=trace_task_id)
    ...
    if skeleton_length_mm is not None:
        if verification_pass:
            summary_message += f"*Skeleton Length:* {skeleton_length_mm:.2f} mm (minimum: {min_skeleton_length_mm} mm) ✅\n"
        else:
            summary_message += f"*Skeleton Length:* {skeleton_length_mm:.2f} mm (minimum: {min_skeleton_length_mm} mm) ❌\n"
            summary_message += f"⚠️ *BELOW MINIMUM REQUIREMENT*\n"
    else:
        summary_message += f"*No skeleton found in segmentation* ⚠️\n"
    ...
    if slack_channel:
        try:
            ...
            if not verification_pass and slack_users:
                ...
                thread_message = f"{user_mentions} This task failed skeleton verification. Please review."
                ...
                console.print(f"[yellow]📢 Tagged users in thread: {', '.join(slack_users)}[/yellow]")
    ...
    if skeleton_info:
        ...
        console.print(f"[yellow]⚠️  No skeleton found in neuroglancer state[/yellow]")
...
def get_task_timesheet_summary(project_name: str, task_id: str) -> dict:
    ...
    with get_session_context() as session:
        ...
        for timesheet in timesheets:
            ...
            user_breakdown[timesheet.user] = user_breakdown.get(timesheet.user, 0) + timesheet.seconds_spent
...
def run_worker(user_id: str, project_name: str, polling_period: float, min_skeleton_length_mm: int, 
               slack_channel: str, graphene_path: str, ng_resolution: tuple, sv_resolution: tuple, slack_users: tuple):
    """Run the segmentation auto verifier worker."""
    # Display startup banner
    users_info = f"[yellow]Slack Users to Tag:[/yellow] {', '.join(slack_users) if slack_users else 'None'}\n" if slack_channel else ""
    startup_panel = Panel(
        ...
        while True:
            ...
            try:
                ...
                if task_id:
                    ...
                    process_task(task_id, project_name, user_id, 
                               min_skeleton_length_mm, slack_channel, task_count,
                               graphene_path, ng_resolution, sv_resolution, slack_users)
