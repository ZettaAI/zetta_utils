...
def test_getitem_single_contact():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        backend = make_backend(temp_dir)
        contact = make_seg_contact(contact_id=42, com=(100.0, 100.0, 100.0))
...
def test_getitem_multiple_contacts():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        backend = make_backend(temp_dir)
        contacts = [
            make_seg_contact(contact_id=1, com=(100.0, 100.0, 100.0)),
            make_seg_contact(contact_id=2, com=(200.0, 200.0, 200.0)),
            make_seg_contact(contact_id=3, com=(300.0, 300.0, 300.0)),
        ]
...
def test_getitem_filters_by_bbox():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        contacts = [
            make_seg_contact(contact_id=1, com=(100.0, 100.0, 100.0)),  # in query
            make_seg_contact(contact_id=2, com=(2000.0, 100.0, 100.0)),  # outside query in x
        ]
        backend.write_chunk((0, 0, 0), contacts)
...
def test_setitem_distributes_to_chunks():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        contacts = [
            make_seg_contact(contact_id=1, com=(100.0, 100.0, 100.0)),
            make_seg_contact(contact_id=2, com=(5000.0, 100.0, 100.0)),  # > 4096
        ]
...
def test_layer_with_changes():
    """Test with_changes returns new layer with updated fields."""
    with tempfile.TemporaryDirectory() as temp_dir:
        backend = make_backend(temp_dir)
        layer = VolumetricSegContactLayer(backend=backend, readonly=False)

        new_layer = layer.with_changes(readonly=True)

        assert new_layer.readonly is True
        assert layer.readonly is False  # original unchanged


def test_layer_pformat():
    """Test pformat returns formatted string."""
    with tempfile.TemporaryDirectory() as temp_dir:
        backend = make_backend(temp_dir)
        layer = VolumetricSegContactLayer(backend=backend)

        result = layer.pformat()

        assert "VolumetricSegContactLayer" in result
