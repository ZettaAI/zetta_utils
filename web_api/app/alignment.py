...
class ApplyCorrespondencesRequest(BaseModel):
    ...
    src_mask: list[list[list[list[float]]]] | None = Field(
        ...
        "Interpretation depends on mask_type.",
    )
    tgt_mask: list[list[list[list[float]]]] | None = Field(
        None,
        description="Binary mask for target, shape (1, H, W, 1). "
        "Interpretation depends on mask_type.",
    )
...
def _parse_json_request(body: dict, device: torch.device):
    ...
    if req.tgt_mask is not None:
        tgt_mask_tensor = torch.tensor(req.tgt_mask, dtype=torch.float32, device=device)
        if req.mask_type == "defect":
...
def _build_json_response(
    ...
    misd_b64 = (
        base64.b64encode(misd_image_np.tobytes()).decode()
...
def _run_misd_detection(warped_image, tgt_image_tensor, input_dtype):
    ...
    t2 = time.time()
    warped_int8 = (
        (warped_image * 127).clamp(-128, 127).round().to(torch.int8)
    )
    tgt_int8 = (
        (tgt_image_tensor * 127).clamp(-128, 127).round().to(torch.int8)
    )
    misd_mask = misd(warped_int8, tgt_int8)
    zero_mask = (
            (warped_int8 == 0).all(dim=0, keepdim=True)
            | (tgt_int8 == 0).all(dim=0, keepdim=True)
    )
    ...
    if input_dtype == torch.int8:
        misd_as_input = (
            (misd_float * 255 - 128).clamp(-128, 127).to(torch.int8)
        )
...
async def apply_correspondences(request: Request):
    ...
    misd_image_np = (
        misd_as_input.cpu().numpy().astype(np.float32)
        if misd_as_input is not None
        else None
    )
