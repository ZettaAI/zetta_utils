...
def _do_stepwise_rounded_div(value: float, factor: float, mode: Literal["ceil", "floor"]) -> float:
    if factor <= 1:
        return value / factor
    elif mode == "ceil":
        return _do_stepwise_rounded_div(math.ceil(value / 2), factor / 2, mode)
    else:
        return _do_stepwise_rounded_div(math.floor(value / 2), factor / 2, mode)
