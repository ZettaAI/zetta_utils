...
class AcedRelaxationOp:
    ...
    def __call__(
        ...
        rigidity_scales: Sequence[int] | None = None,
        ...
        initial_field: VolumetricLayer | None = None,
        num_iter: int = 100,
        lr: float = 0.3,
        rigidity_weight: float = 10.0,
        min_rigidity_multiplier: float = 0.0,
        fix: Literal["first", "last", "both"] | None = None,
        max_dist: int = 2,
    ):
        idx_padded = idx.padded(self.crop_pad)
        ...
        if initial_field is not None or match_offsets_data.any():
            with semaphore("cuda"):
                result = alignment.aced_relaxation.perform_aced_relaxation(
                    ...
                    rigidity_scales=tuple(rigidity_scales) if rigidity_scales else (1,),
                    ...
                    initial_field=(initial_field[idx_padded] if initial_field else None),
                    num_iter=num_iter,
                    lr=lr,
