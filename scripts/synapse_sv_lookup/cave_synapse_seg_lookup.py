...
SYNAPSE_TABLE = "opl_synapses"
SUPERVOX_TABLE = "opl_synapses__dacey_human_fovea_2404"

# Segmentation parameters
# pylint: disable-next=line-too-long
SEG_PATH = "gs://dacey-human-retina-001-segmentation/240320-retina-finetune-embd24-v1-x1/20240403/seg_agg25"
RESOLUTION = Vec3D(20, 20, 50)
SEG_CHUNK_SIZE = Vec3D(256, 256, 64)
SEG_OFFSET = Vec3D(0, 512, 11)
...
def chunk_index_for_point(point):
    """
    Return a VolumetricIndex representing the chunk containing the
    given (Vec3D) point, in voxel coordinates.
    """
    offset_point = point - SEG_OFFSET
    chunk_count = floor(offset_point / SEG_CHUNK_SIZE)
    chunk_start = SEG_OFFSET + chunk_count * SEG_CHUNK_SIZE
    return VolumetricIndex.from_coords(chunk_start, chunk_start + SEG_CHUNK_SIZE, RESOLUTION)
...
def lookup_segment_id(point: Vec3D, load_data_if_needed: bool = False):
    """
    Try to look up the given point (in global voxel coordinates) in
    ...
    if chunk is None or not chunk_bounds.contains(point):  # type: ignore[unreachable]
        if not load_data_if_needed:
            return None
        chunk_bounds = chunk_index_for_point(point)
        chunk = seg_layer[chunk_bounds][0]
    relative_point = floor(point - chunk_bounds.start)
