...
def test_create_task_success_sql(db_session, project_name, sample_task):
    result = create_task(db_session, project_name, sample_task)
    assert result == sample_task["task_id"]

    task = get_task(db_session, project_name, "task_1")
    assert task == sample_task


def test_get_task_success_sql(db_session, project_name, sample_task):
    # First create the task
    create_task(db_session, project_name, sample_task)

    # Then retrieve it
    result = get_task(db_session, project_name, "task_1")
    assert result == sample_task


def test_get_task_not_found_sql(db_session, project_name):
    with pytest.raises(KeyError, match="Task task_1 not found"):
        get_task(db_session, project_name, "task_1")


def test_update_task_success_sql(db_session, project_name, sample_task):
    # First create the task
    create_task(db_session, project_name, sample_task)

    # Then update it
    update_data = TaskUpdate(**{"status": "ingested"})
    result = update_task(db_session, project_name, "task_1", update_data)
...
def test_update_task_invalid_status_sql(db_session, project_name, sample_task):
    # First create the task
    create_task(db_session, project_name, sample_task)

    # Then try to update with invalid status
    with pytest.raises(ValueError, match="Invalid status value"):
        update_task(db_session, project_name, "task_1", TaskUpdate(**{"status": "invalid_status"}))


def test_create_task_duplicate_sql(db_session, project_name):
    task_data = Task(
        **{
    ...
    # First create
    result = create_task(db_session, project_name, task_data)
    assert result == "duplicate_task"

    # Try to create again - should fail
    with pytest.raises(ValueError, match="Task duplicate_task already exists"):
        create_task(db_session, project_name, task_data)

    # Verify the task exists with correct data
    task = get_task(db_session, project_name, "duplicate_task")
    assert task["batch_id"] == "batch_1"
    assert task["status"] == "pending_ingestion"
    assert task["ng_state"] == "http://example.com/duplicate_task"


def test_update_task_not_found_sql(db_session, project_name):
    update_data = TaskUpdate(**{"status": "ingested"})

    with pytest.raises(KeyError, match="Task non_existent_task not found"):
        update_task(db_session, project_name, "non_existent_task", update_data)


# Original Firestore tests


def test_create_task_success(project_name, sample_task):
    result = create_task(project_name=project_name, data=sample_task)
    assert result == sample_task["task_id"]

    task = get_task(project_name=project_name, task_id="task_1")
    assert task == sample_task


def test_get_task_success(existing_task, project_name):
    result = get_task(project_name=project_name, task_id="task_1")
    assert result == existing_task


def test_get_task_not_found(project_name):
    with pytest.raises(KeyError, match="Task task_1 not found"):
        get_task(project_name=project_name, task_id="task_1")


def test_update_task_success(existing_task, project_name):
    update_data = TaskUpdate(**{"status": "ingested"})

    result = update_task(project_name=project_name, task_id="task_1", data=update_data)
    assert result is True
...
def test_task_completion_when_all_subtasks_done(
    ...
    update_task(project_name=project_name, task_id="task_1", data={"status": "ingested"})
...
def test_create_task_validation(project_name):
    ...
    with pytest.raises(Exception):
        create_task(project_name=project_name, data=invalid_task)  # type: ignore
...
def test_update_task_invalid_status(existing_task, project_name):
    with pytest.raises(ValueError, match="Invalid status value"):
        update_task(
            ...
            task_id="task_1",
            data=TaskUpdate(**{"status": "invalid_status"}),
        )
...
def test_create_task_duplicate(project_name):
    ...
    result = create_task(project_name=project_name, data=task_data)
    assert result == "duplicate_task"

    with pytest.raises(ValueError, match="Task duplicate_task already exists"):
        create_task(project_name=project_name, data=task_data)

    task = get_task(project_name=project_name, task_id="duplicate_task")
    assert task["batch_id"] == "batch_1"
    assert task["status"] == "pending_ingestion"
    assert task["ng_state"] == "http://example.com/duplicate_task"
...
def test_update_task_not_found(project_name):
    ...
    with pytest.raises(KeyError, match="Task non_existent_task not found"):
        update_task(project_name=project_name, task_id="non_existent_task", data=update_data)

    with pytest.raises(KeyError, match="Task non_existent_task not found"):
        get_task(project_name=project_name, task_id="non_existent_task")
...
def test_update_task(project_name):
    ...
    update_task(project_name=project_name, task_id="task_2", data=update_data)

    task = get_task(project_name=project_name, task_id="task_2")
    assert task["status"] == "ingested"
