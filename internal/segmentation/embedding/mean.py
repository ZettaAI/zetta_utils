...
def _zero(device: torch.device):
    return torch.zeros(1).to(device).squeeze()
...
class MeanLoss(nn.Module):
    ...
    def compute_loss_int(
        ...
        for vec, mean, weight in zip(vecs, means, weights):
            ...
            loss += weight * torch.mean(torch.max(margin, _zero(device)) ** 2)
    ...
    def compute_loss_ext(
        ...
        if (count > 1) and (mext is not None):
            ...
            margin = margin[mext]
            loss = torch.sum(torch.max(margin, _zero(device)) ** 2)
