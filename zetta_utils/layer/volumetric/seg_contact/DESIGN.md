...
```json
{
  ...
  "ground_truth_path": "gs://bucket/ground_truth",
...
| `segmentation_path` | string? | Path to candidate segmentation layer |
| `ground_truth_path` | string? | Path to ground truth segmentation layer |
...
├── info_spec.py    # SegContactInfoSpec for layer creation/update
└── procs.py        # Data processors (normalize, noise, rotation)
```
