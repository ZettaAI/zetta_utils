...
def release_task(
    *,
    ...
    with get_session_context(db_session) as session:
        ...
        # This prevents race conditions in job completion detection
        task.active_user_id = ""
...
def _handle_task_completion(  # pylint: disable=too-many-locals
    ...
    # Count incomplete tasks for this job
    # Since we've already marked the current task as complete (before calling this function),
    # we can simply count all incomplete tasks without exclusions
    ...
    # If no incomplete tasks remain, mark job as fully_processed
    if incomplete_count == 0:
        logger.info(f"All tasks complete for job {job_id}, marking as fully_processed")
