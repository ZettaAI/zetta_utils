"""Tests for split edit functionality"""
...
def sample_split_data():
    """Sample split edit data"""
    return {
        "sources": [
            ["74450544596533638", 301477.96875, 72537.375, 151875.28125],
            ["74450544596530907", 301440.8125, 72958.609375, 151875.28125],
        ],
        "sinks": [
            ["74450544596522393", 300431.0625, 72072.7734375, 151875.28125],
            ["74450544596525136", 300598.34375, 72122.3359375, 151875.28125],
        ],
    }


class TestSplitEdit:
    """Test cases for split edit functionality"""

    def test_create_split_edit_success(
        self, test_project, test_user, test_task, sample_split_data, db_session
    ):
        """Test successful creation of split edit"""
        edit_id = create_split_edit(
            project_name=test_project,
            task_id=test_task["task_id"],
            user_id=test_user["user_id"],
            sources=sample_split_data["sources"],
    ...
    def test_create_split_edit_returns_different_ids(
        self, test_project, test_user, test_task, sample_split_data, db_session
    ):
        """Test that multiple split edits get different IDs"""
    ...
    def test_get_split_edits_by_task(
        ...
        """Test getting split edits by task ID"""
        # Create split edit
        edit_id = create_split_edit(
            project_name=test_project,
            task_id=test_task["task_id"],
        ...
        assert edits[0]["sources"] == sample_split_data["sources"]
        assert edits[0]["sinks"] == sample_split_data["sinks"]
        assert "created_at" in edits[0]

    def test_get_split_edits_by_user(
        self, test_project, test_user, test_task, sample_split_data, db_session
    ):
        """Test getting split edits by user ID"""
        # Create split edit
        edit_id = create_split_edit(
            project_name=test_project,
            task_id=test_task["task_id"],
    ...
    def test_get_split_edit_by_id(
        self, test_project, test_user, test_task, sample_split_data, db_session
    ):
        """Test getting split edit by ID"""
        # Create split edit
        edit_id = create_split_edit(
            project_name=test_project,
            task_id=test_task["task_id"],
        ...
        assert edit["sources"] == sample_split_data["sources"]
        assert edit["sinks"] == sample_split_data["sinks"]

    def test_get_split_edit_by_id_not_found(
        self, test_project, db_session
    ):
        """Test getting non-existent split edit by ID"""
        edit = get_split_edit_by_id(
            project_name=test_project,
            edit_id=999,
            db_session=db_session,
    ...
    def test_get_split_edits_empty_results(
        ...
        """Test getting split edits when none exist"""
        # Get by task ID
        edits_by_task = get_split_edits_by_task(
            project_name=test_project,
    ...
    def test_split_edits_ordering(
        self, test_project, test_user, test_task, sample_split_data, db_session
    ):
        """Test that split edits are returned in correct order (newest first)"""
        # Create first split edit
        edit_id_1 = create_split_edit(
            project_name=test_project,
            task_id=test_task["task_id"],
            user_id=test_user["user_id"],
    ...
    def test_split_edits_different_projects(
        self, test_user, test_task, sample_split_data, db_session
    ):
        """Test that split edits are properly isolated by project"""
        # Create two projects
        project_1 = "project_1"
        project_2 = "project_2"
