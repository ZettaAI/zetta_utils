...
@builder.register("BaseEncoderCoarsenerRegime")
...
class BaseCoarsenerRegime(pl.LightningModule):  # pylint: disable=too-many-ancestors
    ...
    def compute_metroem_loss(self, batch: dict, mode: str, log_row: bool, sample_name: str = ""):
        ...
        f_warp_small = (
            seed_field * self.field_magn_thr * self.ds_factor / torch.quantile(seed_field.abs().max(1)[0], 0.5)
        )
        ...
        src_enc_f1 = torch.nn.functional.pad(f1_trans, (f_pad,f_pad,f_pad,f_pad), mode='replicate').from_pixels().down(int(log2(self.ds_factor))).sample(src_enc_f1, padding_mode='border')
        ...
        post_magn_mask = ((magn_field.from_pixels().down(int(log2(self.ds_factor))).pixels().abs().max(1)[0]) > self.field_magn_thr).tensor_()
