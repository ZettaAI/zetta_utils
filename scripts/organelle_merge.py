...
seg_vol: Optional[CloudVolume] = None
seg_bounds: Optional[BBox3D] = None
...
def verify_cave_auth():
    ...
    global client
    client = CAVEclient(
        datastack_name="dacey_human_fovea", server_address="https://proofreading.zetta.ai"
    )
    try:
...
def parse_point(point_string: str) -> Optional[Vec3D]:
...
def load_volume(path, scale_index=0) -> tuple[CloudVolume, BBox3D]:
    """
...
def process_line(line: str):
    ...
    if touches_XY_edge_of_block(central_root_id, root_block):
        print(
            f"{central_root_id} at {round(point/inp_to_seg)} extends beyond data window; skipping"
        )
        return
    ...
    print(
        f"{total_neighbors} total neighbors; biggest is {neighbor_root_id} with {neighbor_count} ({percent_covered}%)"
    )
    ...
    neighbor_border_mask = (
        scipy.ndimage.binary_dilation(neighbor_sv_mask, iterations=2) & ~neighbor_sv_mask
    )
    ...
    print(
        f"And THAT has a center at {central_pos}, with SV {sv_slice[central_pos]} and root {root_slice[central_pos]}"
    )

    # Calculate our endpoints, in input coordinates
    assert neighbor_pos is not None
    assert central_pos is not None
    neighbor_point = round(
        Vec3D(dpos[0] + neighbor_pos[0], dpos[1] + neighbor_pos[1], point[2]) / inp_to_seg
    )
    central_point = round(
        Vec3D(dpos[0] + central_pos[0], dpos[1] + central_pos[1], point[2]) / inp_to_seg
    )
...
def do_merge(sv1: int, point1: Vec3D, sv2: int, point2: Vec3D):
    global client, cg_client, pcg_table_name
...
def main():
    ...
    parser.add_argument(
        "-r", "--resolution", help="resolution of point coordinates", required=True
    )
    parser.add_argument("--seg", help="segmentation volume path", required=True)
    parser.add_argument("--sv", help="supervoxel volume path")
    parser.add_argument(
        "--pct", default="90", help="percent of border which must be touched to merge"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="if true, print merges without actually doing them"
    )
    parser.add_argument(
        "--anno", action="store_true", help="if true, output lines/points representing the results"
    )
    ...
    point_resolution = parse_point(args.resolution)  # type: ignore
