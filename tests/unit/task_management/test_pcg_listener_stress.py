...
class TestPCGListenerStress:  # pylint: disable=attribute-defined-outside-init
    ...
    def setup_method(self, method) -> None:
        ...
        self.stress_config = {
            "large_supervoxel_count": 10000,
            "batch_size": 1000,
            "num_segments": 5,
            "supervoxels_per_segment": 1000,
            "message_count": 1000,
            "message_batch_size": 50,
            "memory_test_messages": 100,
            "supervoxel_count_large": 5000,
    ...
    def create_test_supervoxels(self, session, count: int, base_segment_id: int = 100000, base_supervoxel_id: int = 1) -> List[SupervoxelModel]:
    ...
    def test_large_old_supervoxels_collection(self, db_session, project_factory):
        """Test handling of large collections of old_supervoxels."""
        project_factory(project_name=self.project_name)
        
        # Create a large number of supervoxels (stress test with configurable count)
        ...
        assert processing_time < self.stress_config["max_processing_time_seconds"]  # Should complete within configured time
    ...
    def test_concurrent_merge_operations(self, db_session, project_factory):
        ...
        for segment_idx in range(num_segments):
            ...
            for sv_idx in range(supervoxels_per_segment):
                create_supervoxel(
                    supervoxel_id=base_supervoxel_id + segment_idx * supervoxels_per_segment + sv_idx,
    ...
    def test_network_failure_scenarios(self, mocker):  # pylint: disable=unused-argument
        ...
        mock_response = Mock()
        mock_response.raise_for_status.side_effect = requests.Timeout("Connection timeout")
    ...
    def test_large_volume_message_processing(self, mocker):  # pylint: disable=unused-argument
        ...
        for i in range(message_count):
            ...
            messages.append(MockMessage(payload=payload, acknowledge_fn=Mock()))
        
        # Mock API responses
        with patch('zetta_utils.task_management.automated_workers.pcg_edit_listener.get_old_roots_from_lineage_graph') as mock_lineage, \
             patch('zetta_utils.task_management.automated_workers.pcg_edit_listener.get_supervoxel_ids_from_segment') as mock_supervoxels, \
             patch('zetta_utils.task_management.supervoxel.update_supervoxels_for_merge') as mock_merge, \
             patch('zetta_utils.task_management.supervoxel.update_supervoxels_for_split') as mock_split, \
             patch('zetta_utils.task_management.supervoxel.get_supervoxels_by_segment') as mock_get_sv:
            ...
            assert avg_processing_time < self.stress_config["max_batch_time_seconds"]  # Average batch should process in under configured time
    ...
    def test_malformed_message_handling(self, mocker):  # pylint: disable=unused-argument
        ...
        with patch('zetta_utils.task_management.supervoxel.update_supervoxels_for_merge') as mock_merge, \
             patch('zetta_utils.task_management.supervoxel.update_supervoxels_for_split') as mock_split:
    ...
    def test_memory_usage_large_split(self, db_session, project_factory):
        ...
        assert memory_increase < self.stress_config["max_memory_increase_mb"]  # Should not increase by more than configured MB
    ...
    def test_database_deadlock_handling(self, db_session, project_factory):
        ...
        with ThreadPoolExecutor(max_workers=self.stress_config["concurrent_workers"]) as executor:
            futures = [executor.submit(concurrent_operation, i) for i in range(self.stress_config["concurrent_workers"])]
    ...
    def test_event_idempotency_stress(self, db_session, project_factory):
        ...
        assert len(zero_results) >= 4  # At least 4 should be skipped due to idempotency
