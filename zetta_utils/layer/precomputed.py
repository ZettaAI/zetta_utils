...
class InfoSpecParams:
    ...
    num_channels: int
    voxel_offset: Sequence[float]
    size: Sequence[float]
    bounds_resolution: Sequence[float]
    ...
    def from_optional_reference(
        ...
        voxel_offset: Sequence[float] | None = None,
        size: Sequence[float] | None = None,
        bounds_resolution: Sequence[float] | None = None,
        ...
        if reference_path is None:
            if (
                ...
                or encoding is None
                or voxel_offset is None
                or size is None
                or bounds_resolution is None
            ):
                missing_params = []
                if type is None:
                ...
                if voxel_offset is None:
                    missing_params.append("voxel_offset")
                if size is None:
                    missing_params.append("size")
                if bounds_resolution is None:
                    missing_params.append("bounds_resolution")
    ...
    def from_reference(
        ...
        :param voxel_offset: Voxel offset. If inherit_all_params is True, this can be
            None.
        :param size: Size of the volume. If inherit_all_params is True, this can be
            None.
        :param bounds_resolution: Resolution of the bounds (`voxel_offset` and `size`).
            If inherit_all_params is True, this can be None.
        :param inherit_all_params: If True, inherit all unspecified parameters from the
            reference info file. If False, all parameters must be specified.
        ...
        if voxel_offset is None or size is None or bounds_resolution is None:
            if not (voxel_offset is None and size is None and bounds_resolution is None):
                raise ValueError(
                    "Either all of `voxel_offset`, `size`, and `bounds_resolution` must "
                    "be specified or none of them should be specified."
                )
            bounds_resolution = reference_scale["resolution"]
            voxel_offset = reference_scale["resolution"]
            size = reference_scale["size"]
        ...
        assert voxel_offset is not None
        assert size is not None
        assert bounds_resolution is not None
...
class PrecomputedInfoSpec:
    ...
    def make_info(self) -> dict:
        if self.info_path is not None:
            ...
            for scale_resolution in self.info_spec_params.scales:
                scale_dict: dict = {
                    "chunk_sizes": [self.info_spec_params.chunk_size],
                ...
                multiplier = [
                    k / v
                    for k, v in zip(scale_resolution, self.info_spec_params.bounds_resolution)
                ]

                scale_dict["size"] = [
                    math.ceil(k / m) for k, m in zip(self.info_spec_params.size, multiplier)
                ]
                scale_dict["voxel_offset"] = [
                    k / m for k, m in zip(self.info_spec_params.voxel_offset, multiplier)
                ]
    ...
    def set_voxel_offset(self, voxel_offset_and_res: tuple[Vec3D[int], Vec3D[float]]) -> None:
        ...
        assert self.info_spec_params is not None
        self.info_spec_params.voxel_offset = voxel_offset
        self.info_spec_params.scales = list(set(self.info_spec_params.scales).union(set([res])))

    def set_chunk_size(self, chunk_size_and_res: tuple[Vec3D[int], Vec3D]) -> None:
        chunk_size, res = chunk_size_and_res
        assert self.info_spec_params is not None
    ...
    def set_dataset_size(self, dataset_size_and_res: tuple[Vec3D[int], Vec3D]) -> None:
        ...
        assert self.info_spec_params is not None
        self.info_spec_params.size = dataset_size
        self.info_spec_params.scales = list(set(self.info_spec_params.scales).union(set([res])))
