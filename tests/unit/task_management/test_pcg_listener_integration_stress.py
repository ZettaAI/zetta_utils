...
from zetta_utils.task_management.automated_workers.run_pcg_listener import main
...
class TestPCGListenerIntegrationStress:
    ...
    def test_listener_startup_and_shutdown(self, mocker):
        ...
        with patch('zetta_utils.task_management.automated_workers.pcg_edit_listener.get_session_context') as mock_session_ctx, \
             patch('zetta_utils.task_management.automated_workers.pcg_edit_listener.PubSubPullQueue') as mock_queue_class, \
             patch('zetta_utils.task_management.automated_workers.pcg_edit_listener.CAVEclient') as mock_cave_client, \
             patch('zetta_utils.task_management.automated_workers.pcg_edit_listener.process_edit_event') as mock_process_event, \
    ...
    def test_high_message_throughput(self, mocker):
        ...
        with patch('zetta_utils.task_management.automated_workers.pcg_edit_listener.PubSubPullQueue') as mock_queue_class, \
             patch('zetta_utils.task_management.automated_workers.pcg_edit_listener.process_edit_event', side_effect=mock_process_event), \
             patch('time.sleep'):
            ...
            assert total_time < self.stress_config["max_processing_time_seconds"]  # Should complete within configured time
    ...
    def test_database_connection_failures(self, mocker):
        ...
        def mock_process_event(*args, **kwargs):
            ...
            # Simulate processing happens but doesn't need retries for this test
        
        with patch('zetta_utils.task_management.automated_workers.pcg_edit_listener.PubSubPullQueue') as mock_queue_class, \
             patch('zetta_utils.task_management.automated_workers.pcg_edit_listener.get_session_context') as mock_session_ctx, \
             patch('zetta_utils.task_management.automated_workers.pcg_edit_listener.process_edit_event', side_effect=mock_process_event), \
             patch('time.sleep'):
    ...
    def test_cave_client_initialization_failures(self, mocker):
        ...
        def mock_cave_client_init(*args, **kwargs):
            raise Exception("CAVEclient initialization failed")
        ...
        with patch('zetta_utils.task_management.automated_workers.pcg_edit_listener.PubSubPullQueue') as mock_queue_class, \
             patch('zetta_utils.task_management.automated_workers.pcg_edit_listener.CAVEclient', side_effect=mock_cave_client_init), \
             patch('zetta_utils.task_management.automated_workers.pcg_edit_listener.process_edit_event', side_effect=mock_process_event), \
             patch('time.sleep'):
    ...
    def test_memory_leak_detection(self, mocker):
        ...
        def mock_pull(max_num):
            ...
            if message_counter >= self.stress_config["memory_test_messages"]:  # Stop after configured messages
                raise KeyboardInterrupt()
            
            batch = []
            for _ in range(min(max_num, 10)):  # Small batches
                message_counter += 1
                batch.append(MockPubSubMessage({
        ...
        assert memory_increase < self.stress_config["max_memory_increase_mb"], f"Memory increased by {memory_increase}MB"
        assert message_counter >= self.stress_config["memory_test_messages"]  # Verify we processed the expected number of messages

    def test_cli_argument_parsing(self, mocker):
        ...
        test_cases = [
            ...
            [
                ...
                "--datastack-name", "test-datastack",
                "--server-address", "https://test.com",
                "--poll-interval", "10",
                "--max-messages", "5"
            ],
        ...
        with patch('zetta_utils.task_management.automated_workers.run_pcg_listener.run_pcg_edit_listener') as mock_run_listener, \
             patch('zetta_utils.task_management.db.session.get_session_context') as mock_session_ctx:
            ...
            # For other test cases, project name is not provided so no query is made
                ...
                with patch('sys.argv', test_args):
                    try:
                        ...
                        if i == 2:  # Invalid poll interval case
                            ...
                            pytest.fail(f"Unexpected SystemExit for test case {i}: {e}")
                    except Exception as e:
                        if i == 2:  # Invalid poll interval case
                            # Expected for invalid arguments
                            pass
                        else:
                            pytest.fail(f"Unexpected exception for test case {i}: {e}")

    def test_graceful_shutdown_on_signals(self, mocker):
