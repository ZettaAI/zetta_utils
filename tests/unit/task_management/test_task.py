...
def test_create_task_duplicate(project_name_task):
    task_data = Task(
        **{
            ...
            "status": "pending_ingestion",
            "task_type": "segmentation",
            "link": "http://example.com/duplicate_task",
        }
    )
    ...
    assert task["link"] == "http://example.com/duplicate_task"
...
def test_create_task(project_name_task):
    ...
    assert task["batch_id"] == "batch_1"
    assert task["status"] == "pending_ingestion"
    assert task["link"] == "http://example.com/task_1"
...
def test_update_task(project_name_task):
    ...
    update_data = TaskUpdate(
        **{
            "status": "ingested",
            "link": "http://example.com/task_2_updated",
        }
    )
    ...
    assert task["status"] == "ingested"
    assert task["link"] == "http://example.com/task_2_updated"
