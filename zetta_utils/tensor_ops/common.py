...
class DictSupportingTensorOp(Generic[P]):
    ...
    @overload
    def __call__(
        self, data: TensorTypeVar, *args: P.args, targets: Container[str] | None, **kwargs: P.kwargs
    ) -> TensorTypeVar:
        ...

    @overload
    def __call__(
        self, data: Mapping[Any, TensorTypeVar],
        *args: P.args, targets: Container[str] | None, **kwargs: P.kwargs
        ...
        if isinstance(data, Mapping):
            for k in data.keys():
                if targets is None or k in targets:
                    data[k] = self.fn(data[k], *args, **kwargs)
            return data
        else:
