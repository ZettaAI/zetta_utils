...
class SegmentMergeEventModel(Base):
    """
    SQLAlchemy model for the segment_merge_events table.

    Tracks merge/split events from PyChunkedGraph to provide audit trail
    and enable idempotent processing of PubSub messages.
    """

    __tablename__ = "segment_merge_events"
    ...
    new_root_id: Mapped[int] = mapped_column(BigInteger, nullable=False)
    ...
    __table_args__ = (
        Index("idx_merge_events_new_root", "new_root_id"),
        Index("idx_merge_events_timestamp", "edit_timestamp"),
        Index("idx_merge_events_processed", "processed_at"),
    )
    ...
    @classmethod
    def from_dict(cls, data: dict) -> "SegmentMergeEventModel":
        """Create a model instance from a dictionary"""
        return cls(
            project_name=data["project_name"],
            event_id=data["event_id"],
