...
class TestTaskFeedbackModelDatabase:
    ...
    def test_create_task_feedback_model(self, clean_db, db_session, project_name):
        ...
        result = db_session.query(TaskFeedbackModel).filter(
            TaskFeedbackModel.project_name == project_name,
            TaskFeedbackModel.feedback_id == 1
        ).first()
    ...
    def test_task_feedback_model_primary_key(self, clean_db, db_session, project_name):
        ...
        # Query both
        results = db_session.query(TaskFeedbackModel).filter(
            TaskFeedbackModel.project_name == project_name
        ).all()
    ...
    def test_task_feedback_model_foreign_key_constraint(
        self, clean_db, db_session, project_name, sample_original_task, sample_feedback_task,
        sample_trace_task_type, sample_feedback_task_type
    ):
        """Test TaskFeedbackModel with existing tasks"""
        # Create task types
        create_task_type(project_name=project_name, data=sample_trace_task_type, db_session=db_session)
        create_task_type(project_name=project_name, data=sample_feedback_task_type, db_session=db_session)
        ...
        result = db_session.query(TaskFeedbackModel).join(
            TaskModel, 
    ...
    def test_task_feedback_model_bulk_operations(self, clean_db, db_session, project_name):
        ...
        # Verify deletion
        remaining = db_session.query(TaskFeedbackModel).filter(
            TaskFeedbackModel.project_name == project_name
        ).count()
    ...
    def test_task_feedback_model_query_performance(self, clean_db, db_session, project_name):
        ...
        results = db_session.query(TaskFeedbackModel).filter(
            TaskFeedbackModel.project_name == project_name
        ).order_by(TaskFeedbackModel.created_at.desc()).limit(20).all()
