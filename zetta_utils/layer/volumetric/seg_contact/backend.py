...
class SegContactLayerBackend(Backend[VolumetricIndex, Sequence[SegContact], Sequence[SegContact]]):
    ...
    def write_chunk(self, chunk_idx: tuple[int, int, int], contacts: Sequence[SegContact]) -> None:
        ...
        pointclouds_by_config: dict[str, list[tuple[int, int, int, np.ndarray, np.ndarray]]] = {}
        for contact in contacts:
            if contact.local_pointclouds is not None:
                for config_key, seg_points in contact.local_pointclouds.items():
                    if config_key not in pointclouds_by_config:
                        pointclouds_by_config[config_key] = []
                    ...
                    if seg_a_pts is not None and seg_b_pts is not None:
                        pointclouds_by_config[config_key].append(
                            (contact.id, contact.seg_a, contact.seg_b, seg_a_pts, seg_b_pts)
                        )

        for config_key, entries in pointclouds_by_config.items():
            self._write_pointcloud_chunk(chunk_idx, config_key, entries)
    ...
    def _write_contacts_chunk(
        ...
        CloudFile(chunk_path).put(buf.getvalue())
    ...
    def _write_pointcloud_chunk(
        ...
        """
        # Parse config key: "r{radius}_n{n_points}"
        parts = config_key.split("_")
        radius_nm = float(parts[0][1:])
        n_points = int(parts[1][1:])
    ...
    def read_chunk(  # pylint: disable=too-many-locals
        ...
        if cf.exists():
            info = json.loads(cf.get().decode("utf-8"))

            # Read pointcloud configs from info and load any that exist
            pointcloud_configs = info.get("local_point_clouds", [])
            for cfg in pointcloud_configs:
                ...
                for contact_id, seg_a_pts, seg_b_pts in pc_data:
                    if contact_id in contact_lookup:
                        ...
                        if c.local_pointclouds is None:
                            c.local_pointclouds = {}
                        c.local_pointclouds[config_key] = {
                            c.seg_a: seg_a_pts,
                            c.seg_b: seg_b_pts,
                        }
    ...
    def _read_contacts_chunk(self, chunk_idx: tuple[int, int, int]) -> list[dict]:
        ...
        chunk_path = self.get_chunk_path(chunk_idx)
        cf = CloudFile(chunk_path)
        if not cf.exists():
            return []

        contacts = []
        with io.BytesIO(cf.get()) as f:
