...
def test_write_chunk_creates_file():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        contact = make_seg_contact(contact_id=1, seg_a=100, seg_b=200, com=(100.0, 100.0, 100.0))
...
def test_write_read_chunk_single_contact():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        contact = make_seg_contact(
            contact_id=42, seg_a=100, seg_b=200, com=(100.0, 200.0, 300.0), n_faces=5
        )
        backend.write_chunk((0, 0, 0), [contact])
...
def test_write_read_chunk_multiple_contacts():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        contacts = [
            make_seg_contact(
                contact_id=1, seg_a=100, seg_b=200, com=(100.0, 100.0, 100.0), n_faces=3
            ),
            make_seg_contact(
                contact_id=2, seg_a=100, seg_b=300, com=(200.0, 200.0, 200.0), n_faces=7
            ),
            make_seg_contact(
                contact_id=3, seg_a=200, seg_b=300, com=(300.0, 300.0, 300.0), n_faces=1
            ),
        ]
...
def test_write_distributes_contacts_to_chunks():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        contacts = [
            make_seg_contact(contact_id=1, seg_a=100, seg_b=200, com=(100.0, 100.0, 100.0)),
            make_seg_contact(
                contact_id=2, seg_a=100, seg_b=300, com=(5000.0, 100.0, 100.0)
            ),  # > 4096
        ]
...
def test_read_filters_by_bbox():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        contacts = [
            make_seg_contact(contact_id=1, seg_a=100, seg_b=200, com=(100.0, 100.0, 100.0)),
            make_seg_contact(contact_id=2, seg_a=100, seg_b=300, com=(2000.0, 100.0, 100.0)),
            make_seg_contact(contact_id=3, seg_a=200, seg_b=300, com=(3500.0, 100.0, 100.0)),
        ]
        backend.write_chunk((0, 0, 0), contacts)
...
def test_read_spans_multiple_chunks():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        backend.write_chunk(
            (1, 0, 0),
            [
                make_seg_contact(contact_id=2, seg_a=100, seg_b=300, com=(5000.0, 100.0, 100.0)),
            ],
        )
...
def test_round_trip_full():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        contacts = [
            make_seg_contact(
                contact_id=1, seg_a=100, seg_b=200, com=(100.0, 100.0, 100.0), n_faces=5
            ),
            make_seg_contact(
                contact_id=2, seg_a=100, seg_b=300, com=(200.0, 200.0, 200.0), n_faces=10
            ),
        ]
...
def test_pointclouds_written_to_separate_directory():
    """Test that local_pointclouds are written to local_point_clouds/{config}/ directory."""
    with tempfile.TemporaryDirectory() as temp_dir:
        backend = SegContactLayerBackend(
            path=temp_dir,
            resolution=Vec3D(16, 16, 40),
        ...
        assert os.path.exists(pointcloud_dir), f"Expected {pointcloud_dir} to exist"

        chunk_file = os.path.join(pointcloud_dir, "0-256_0-256_0-128")
        assert os.path.exists(chunk_file), f"Expected {chunk_file} to exist"


def test_merge_decisions_written_to_separate_directory():
    """Test that merge_decisions are written to merge_decisions/{authority}/ directory."""
    with tempfile.TemporaryDirectory() as temp_dir:
        backend = SegContactLayerBackend(
            path=temp_dir,
            resolution=Vec3D(16, 16, 40),
        ...
        for authority in ["ground_truth", "model_v1"]:
            authority_dir = os.path.join(temp_dir, "merge_decisions", authority)
...
def test_contacts_directory_has_no_pointclouds():
    """Test that contacts/ directory does not contain pointcloud data."""
    with tempfile.TemporaryDirectory() as temp_dir:
        backend = SegContactLayerBackend(
            path=temp_dir,
            resolution=Vec3D(16, 16, 40),
        ...
        pointcloud_a = np.random.rand(1000, 3).astype(np.float32)
        pointcloud_b = np.random.rand(1000, 3).astype(np.float32)
        ...
        # Should be under 200 bytes without embedded pointclouds
        assert (
            file_size < 200
        ), f"Contact chunk too large ({file_size} bytes), pointclouds may be embedded"


def test_round_trip_with_pointclouds_and_decisions():
    """Test round-trip with local_pointclouds and merge_decisions via separate dirs."""
    with tempfile.TemporaryDirectory() as temp_dir:
        backend = SegContactLayerBackend(
            path=temp_dir,
            resolution=Vec3D(16, 16, 40),
        ...
        # Write info file with pointcloud and merge_decisions configs
        info = {
            "format_version": "1.0",
            "type": "seg_contact",
            "resolution": [16, 16, 40],
        ...
        # Verify pointclouds are read back correctly
        assert c.local_pointclouds is not None
        assert (200, 100) in c.local_pointclouds
        np.testing.assert_array_almost_equal(c.local_pointclouds[(200, 100)][100], pointcloud_a)
        np.testing.assert_array_almost_equal(c.local_pointclouds[(200, 100)][200], pointcloud_b)

        # Verify merge decisions are read back correctly
        assert c.merge_decisions == {"ground_truth": True}


# --- Property and method tests for coverage ---


def test_backend_name_property():
    """Test that name property returns path."""
    backend = SegContactLayerBackend(
        path="/tmp/test_path",
        resolution=Vec3D(16, 16, 40),
        voxel_offset=Vec3D(0, 0, 0),
    ...
    assert backend.name == "/tmp/test_path"


def test_backend_with_changes():
    """Test with_changes returns new backend with updated fields."""
    backend = SegContactLayerBackend(
        path="/tmp/test_path",
        resolution=Vec3D(16, 16, 40),
        voxel_offset=Vec3D(0, 0, 0),
    ...
    new_backend = backend.with_changes(path="/tmp/new_path", max_contact_span=1024)

    assert new_backend.path == "/tmp/new_path"
    assert new_backend.max_contact_span == 1024
    # Original unchanged
    assert backend.path == "/tmp/test_path"
    assert backend.max_contact_span == 512


def test_backend_read_info():
    """Test read_info method returns info dict."""
    with tempfile.TemporaryDirectory() as temp_dir:
        backend = SegContactLayerBackend(
            path=temp_dir,
            resolution=Vec3D(16, 16, 40),
...
def test_backend_get_pointcloud_configs():
    """Test get_pointcloud_configs returns list of (radius_nm, n_points)."""
    with tempfile.TemporaryDirectory() as temp_dir:
        backend = SegContactLayerBackend(
            path=temp_dir,
            resolution=Vec3D(16, 16, 40),
        ...
        # Write info with pointcloud configs
        info = {
            "format_version": "1.0",
            "type": "seg_contact",
            "resolution": [16, 16, 40],
        ...
        configs = backend.get_pointcloud_configs()
        assert configs == [(500, 64), (1000, 128)]


def test_backend_get_pointcloud_configs_empty():
    """Test get_pointcloud_configs returns empty list when no configs."""
    with tempfile.TemporaryDirectory() as temp_dir:
        backend = SegContactLayerBackend(
            path=temp_dir,
            resolution=Vec3D(16, 16, 40),
...
def test_get_contact_counts_empty_region():
    """Test get_contact_counts returns empty dict for empty bbox."""
    with tempfile.TemporaryDirectory() as temp_dir:
        backend = SegContactLayerBackend(
            path=temp_dir,
            resolution=Vec3D(16, 16, 40),
        ...
        # Create an index with zero-sized bbox (same start and end)
        # This should result in empty relative_paths
        idx = VolumetricIndex(
            resolution=Vec3D(16, 16, 40),
            bbox=BBox3D.from_coords([0, 0, 0], [0, 0, 0], [16, 16, 40]),
        )
        counts = backend.get_contact_counts(idx)
        assert not counts


def test_read_pointcloud_chunk_nonexistent_returns_empty():
    """Test _read_pointcloud_chunk returns empty list when file doesn't exist."""
    with tempfile.TemporaryDirectory() as temp_dir:
        backend = SegContactLayerBackend(
            path=temp_dir,
            resolution=Vec3D(16, 16, 40),
        ...
        result = backend._read_pointcloud_chunk((0, 0, 0), 500.0, 64)
        assert not result


def test_read_merge_decision_chunk_nonexistent_returns_empty():
    """Test _read_merge_decision_chunk returns empty list when file doesn't exist."""
    with tempfile.TemporaryDirectory() as temp_dir:
        backend = SegContactLayerBackend(
            path=temp_dir,
            resolution=Vec3D(16, 16, 40),
