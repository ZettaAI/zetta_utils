...
class BaseCoarsenerRegime(pl.LightningModule):  # pylint: disable=too-many-ancestors
    ...
    post_weight_multiplier: float = 1.0
    zero_value: float = 0
    ...
    def training_step(self, batch, batch_idx):  # pylint: disable=arguments-differ
        ...
        with torchfields.set_identity_mapping_cache(True, clear_cache=False):
            losses = [
                self.compute_metroem_loss(batch=batch, mode="train", log_row=log_row, apply_count=count) for count in self.apply_counts
            ]
    ...
    def compute_metroem_loss(self, batch: dict, mode: str, log_row: bool, sample_name: str = "", apply_count: int = 1):
        ...
        f_warp_small = (
            seed_field * self.field_magn_thr * self.ds_factor ** apply_count / torch.quantile(seed_field.abs().max(1)[0], 0.5)
        )
        ...
        src_zeros_f1 = self._down_zeros_mask(src_zeros_f1, count=int(log2(self.ds_factor ** apply_count)))
        src_zeros_f2 = self._down_zeros_mask(src_zeros_f2, count=int(log2(self.ds_factor ** apply_count)))
        tgt_zeros_f1 = self._down_zeros_mask(tgt_zeros_f1, count=int(log2(self.ds_factor ** apply_count)))
        ...
        f_pad = self.ds_factor ** apply_count
        src_enc_f1 = torch.nn.functional.pad(src_enc, (1,1,1,1), value=0.0)  # TanH! - fill with output zero value
        src_enc_f1 = torch.nn.functional.pad(f1_trans, (f_pad,f_pad,f_pad,f_pad), mode='replicate').from_pixels().down(int(log2(self.ds_factor ** apply_count))).sample(src_enc_f1, padding_mode='border')
        ...
        post_magn_mask = ((magn_field.from_pixels().down(int(log2(self.ds_factor ** apply_count))).pixels().abs().max(1)[0]) > self.field_magn_thr).tensor_()
        ...
        if post_mask.sum() < (256 // ((2 * self.ds_factor) ** apply_count)):
            return None
        ...
        post_weight = self.post_weight * self.post_weight_multiplier ** (apply_count - 1)
        loss = 4 ** (apply_count - 1) * (pre_loss - post_loss * post_weight + equi_loss * self.equivar_weight)
        self.log(f"loss/{mode}_apply_{apply_count}", loss, on_step=True, on_epoch=True)
        self.log(f"loss/{mode}_apply_{apply_count}_pre", pre_loss, on_step=True, on_epoch=True)
        self.log(f"loss/{mode}_apply_{apply_count}_post", post_loss, on_step=True, on_epoch=True)
        self.log(f"loss/{mode}_apply_{apply_count}_equi", equi_loss, on_step=True, on_epoch=True)
        if log_row:
            self.log_results(
                mode,
                sample_name,
    ...
    def validation_step(self, batch, batch_idx):  # pylint: disable=arguments-differ
        ...
        losses = [
            self.compute_metroem_loss(
                batch=batch, mode="val", log_row=log_row, sample_name=sample_name, apply_count=count
