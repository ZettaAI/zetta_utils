...
# TestUtilityFunctions tests


def test_send_slack_error_notification_success_utility_functions(mocker):
...
def test_send_slack_error_notification_no_channel_utility_functions(mocker):
...
def test_send_slack_error_notification_no_token_utility_functions(mocker):
...
def test_send_slack_error_notification_api_error_utility_functions(mocker):
...
# TestProcessTask tests


def test_process_task_with_errors_process_task(mocker):
...
def test_process_task_no_extra_data_process_task(mocker):
...
def test_process_task_no_seed_id_process_task(mocker):
...
def test_process_task_update_exception_process_task(mocker):
...
def test_process_task_with_general_error_process_task(mocker):
...
def test_run_worker_no_tasks_run_worker(mocker):
    ...
    runner = CliRunner()
    result = runner.invoke(run_worker, ["--user_id", "worker1", "--project_name", "test_project"])
...
def test_run_worker_with_persistent_error_run_worker(mocker):
...
def test_run_worker_error_recovery_run_worker(mocker):
...
def test_run_worker_cli_options_run_worker():
...
def test_run_worker_default_user_id_run_worker(mocker):
