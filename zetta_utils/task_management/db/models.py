...
from sqlalchemy import ARRAY, JSON, BigInteger, Boolean, DateTime, Enum, Float, ForeignKey, Index, Integer, String
...
class SegmentTypeModel(Base):
    ...
    reference_segment_ids: Mapped[list[int]] = mapped_column(ARRAY(Integer), nullable=False, default=[])
...
class SegmentModel(Base):
    ...
    project_name: Mapped[str] = mapped_column(String, primary_key=True)
    seed_sv_id: Mapped[int] = mapped_column(BigInteger, primary_key=True)
    ...
    status: Mapped[str] = mapped_column(Enum('wip', 'complete', 'retired', 'abandoned', name='segment_status'), nullable=False, default='wip')
    ...
    def to_dict(self) -> dict:
        ...
        if self.seed_sv_id is not None:
            result["seed_sv_id"] = self.seed_sv_id
    ...
    def from_dict(cls, data: dict) -> "SegmentModel":
        ...
        return cls(
            ...
            seed_sv_id=data.get("seed_sv_id"),
...
class EndpointModel(Base):
    ...
    status: Mapped[str] = mapped_column(Enum('CERTAIN', 'UNCERTAIN', 'CONTINUED', 'BREADCRUMB', name='endpoint_status'), nullable=False)
    user: Mapped[str] = mapped_column(String, nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    
    __table_args__ = (
        Index("idx_endpoints_project_segment", "project_name", "seed_sv_id"),
...
class EndpointUpdateModel(Base):
    ...
    user: Mapped[str] = mapped_column(String, nullable=False)
    new_status: Mapped[str] = mapped_column(Enum('CERTAIN', 'UNCERTAIN', 'CONTINUED', 'BREADCRUMB', name='endpoint_status'), nullable=False)
