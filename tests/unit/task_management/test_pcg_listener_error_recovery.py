...
class DatabaseFailureSimulator:
    ...
    def simulate_operation(self, *args, **kwargs):
        ...
        if failure_type == 'connection':
            ...
            raise Exception("Database operation timeout")
...
class TestPCGListenerErrorRecovery:
    ...
    def test_database_failure_recovery(self, mocker):
        ...
        for pattern in test_patterns:
            ...
            with patch('zetta_utils.task_management.supervoxel.update_supervoxels_for_merge', 
                ...
                try:
                    ...
                    assert "Error" in str(e) or db_simulator.call_count <= len(pattern)

    def test_cascading_failure_scenarios(self, mocker):
        ...
        network_simulator.set_failure_pattern(['timeout', 'connection', 'timeout'])
        ...
        with patch('requests.post', side_effect=network_simulator.simulate_request), \
             patch('zetta_utils.task_management.supervoxel.update_supervoxels_for_split', 
                   side_effect=db_simulator.simulate_operation), \
             patch('zetta_utils.task_management.automated_workers.pcg_edit_listener.get_supervoxels_by_segment', 
                   return_value=[]):
    ...
    def test_partial_failure_recovery(self, db_session, project_factory):
        ...
        # Simulate partial failure: some supervoxels update successfully, others fail
        update_call_count = 0
        def mock_update_with_partial_failure(*args, **kwargs):
            ...
            if update_call_count <= 2:
                ...
                # Third call fails
                raise Exception("Partial update failure")
        ...
        with patch('zetta_utils.task_management.automated_workers.pcg_edit_listener.update_supervoxels_for_merge', 
                   side_effect=mock_update_with_partial_failure), \
             patch('zetta_utils.task_management.automated_workers.pcg_edit_listener.get_old_roots_from_lineage_graph',
                   return_value={}):
            ...
            # Verify attempt was made - function is called because old_root_ids are provided
            assert update_call_count >= 1

    def test_memory_pressure_handling(self, mocker):
        """Test behavior under memory pressure conditions."""
        
        # Simulate memory allocation failures
        allocation_count = 0
        def mock_memory_pressure(*args, **kwargs):
        ...
        with patch('zetta_utils.task_management.automated_workers.pcg_edit_listener.get_supervoxels_by_segment', 
    ...
    def test_concurrent_failure_scenarios(self, mocker):
        ...
        def mock_concurrent_operation(*args, **kwargs):
            ...
            if operation_id % 3 == 0:  # Fail every 3rd operation
                ...
                raise Exception(f"Concurrent operation failure {failure_count}")
            else:
                success_count += 1
                return 1  # Success
        ...
        with patch('zetta_utils.task_management.automated_workers.pcg_edit_listener.update_supervoxels_for_merge', 
            ...
            # Process events sequentially to avoid actual threading issues in tests
    ...
    def test_data_corruption_detection(self, mocker):
        ...
        corruption_scenarios = [
            ...
            # Malformed timestamp
            {"new_root_ids": [123], "timestamp": "not_a_date", "operation_id": "corrupt5"},
            
            # Circular references in old/new root IDs
            {"new_root_ids": [123], "old_root_ids": [123], "operation_id": "corrupt6"},
        ]
    ...
    def test_resource_exhaustion_scenarios(self, mocker):
        """Test behavior under resource exhaustion conditions."""
        
        # Test file descriptor exhaustion
        def mock_fd_exhaustion(*args, **kwargs):
            raise OSError("Too many open files")
        
        # Test disk space exhaustion
        def mock_disk_exhaustion(*args, **kwargs):
            raise OSError("No space left on device")
        
        # Test network socket exhaustion
        def mock_socket_exhaustion(*args, **kwargs):
    ...
    def test_long_running_stability(self, mocker):
        ...
        def mock_long_running_operation(*args, **kwargs):
        ...
        # Verify resilience: account for the fact that with failure, some events may be processed successfully
        assert events_processed > 0
        assert failures_handled >= 0  # At least 0 failures handled
        assert operation_count == 20
        # Total processed events + handled failures should account for all 20 operations
