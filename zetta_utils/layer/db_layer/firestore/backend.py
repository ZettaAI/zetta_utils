...
class FirestoreBackend(DBBackend):
    ...
    def query(
        ...
        `timeout` (seconds): when set, bounds the total deadline of the underlying
            gRPC call (including its internal retries). The Firestore client's
            default deadline is 300s, which is too long for instrumentation paths
            that need to fail fast on transient connectivity issues. Default is
            unset (use library default).
        """
        rpc_kwargs: dict[str, Any] = {}
        if timeout is not None:
            rpc_kwargs["retry"] = Retry(deadline=timeout)
    ...
    def get_batch(
        ...
        TODO: the partitioning logic below occasionally returns the same row in
        two adjacent batches (test_batching flakes with sum != ROW_COUNT). The
        partition `[0, scaled_batch_size) ∪ [scaled_batch_size, sys.maxsize)`
        is exact at the int64 level, but `_id_nonunique` is generated by
        `np.random.randint(sys.maxsize)` which returns `numpy.int64`. The
        Firestore Python client may serialize numpy.int64 as a `double_value`
        instead of `integer_value`, losing precision near `2^63` so values
        near a partition boundary can land in both batches when compared as
        doubles. Production callers (skeletonization shards.py) only do
        idempotent writes so the impact is wasted work, not corruption — but
        a non-idempotent caller would be at risk. Fix options: cap the random
        range to `2**53 - 1` (within double precision), or cast to native
        `int` before storing, or switch to a non-random partitioning scheme.
        """
        if len(self) == 0:
            return {}
