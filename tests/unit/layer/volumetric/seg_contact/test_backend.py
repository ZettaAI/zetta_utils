...
def test_pointclouds_written_to_separate_directory():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        contact = SegContact(
            ...
            contact_faces=np.array([[100, 100, 100, 0.5]], dtype=np.float32),
            local_pointclouds={
                "r200_n100": {100: pointcloud_a, 200: pointcloud_b},
            },
        )
        backend.write_chunk((0, 0, 0), [contact])
...
def test_contacts_directory_has_no_pointclouds():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        contact = SegContact(
            ...
            contact_faces=np.array([[100, 100, 100, 0.5]], dtype=np.float32),
            local_pointclouds={
                "r200_n1000": {100: pointcloud_a, 200: pointcloud_b},
            },
        )
        backend.write_chunk((0, 0, 0), [contact])
...
def test_round_trip_with_pointclouds_and_decisions():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert "r200_n100" in c.local_pointclouds
        np.testing.assert_array_almost_equal(c.local_pointclouds["r200_n100"][100], pointcloud_a)
        np.testing.assert_array_almost_equal(c.local_pointclouds["r200_n100"][200], pointcloud_b)
