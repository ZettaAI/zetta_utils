...
class BoundingCube:
    ...
    bounds: Sequence[Tuple[Union[int, float], Union[int, float]]]
    ...
    def from_slices(
        ...
        for s in slices:
            if s.step is not None:
                raise ValueError(
                    f"Cannot construct a boundingbox from strided slice: '{s}'."
                )
    ...
    def from_coords(
        cls,
        start_coord: Sequence[Union[int, float]],
        end_coord: Sequence[Union[int, float]],
        resolution: Sequence[Union[int, float]],
        unit_name: str = DEFAULT_UNIT,
    ) -> BoundingCube:
        assert_equal_len(
    ...
    def get_slice(
        self,
        dim: int,
        resolution: Union[int, float, Sequence[Union[int, float]]],
    ...
    def to_slices(
        self, resolution: Sequence[Union[int, float]], allow_rounding: bool = False
    ) -> tuple[slice, ...]:
    ...
    def pad(
        ...
        if in_place:
            ...
            result = BoundingCube.from_slices(
                ...
                resolution=[1 for _ in range(self.ndim)],
    ...
    def __repr__(self) -> str:  # pragma: no cover
        ...
        for i in range(self.ndim): # pylint: disable=unused-variable
            parts.append("Dim {i}: {self.bounds[i][0]}-{self.bounds[i][1]}{self.unit_name}")
        parts.append(')')
        result = ''.join(parts)
        return result
